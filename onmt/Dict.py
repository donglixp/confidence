...
class Dict(object):
    ...
    def prune_by_freq(self, min_freq):
        "Return a new dictionary with the entries whose frequency >= `min_freq`."
        ...
        for i in [idx for idx in range(len(self.frequencies)) if self.frequencies[idx] >= min_freq]:
            newDict.add(self.idxToLabel[i])

        return newDict

    def convertToIdx_list(self, labels, unkWord, bosWord=None, eosWord=None):
        vec = []
    ...
    def convertToIdx(self, labels, unkWord, bosWord=None, eosWord=None):
        ...
        return torch.LongTensor(self.convertToIdx_list(labels, unkWord, bosWord, eosWord))
